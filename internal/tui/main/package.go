...
func (p *Package) summary() string {
	if len(p.reports) > 0 {
		...
		for _, state := range []doctor.State{doctor.OK, doctor.Missing, doctor.Replaced, doctor.Foreign, doctor.Unowned, doctor.Unnormalized, doctor.Invisible, doctor.Orphaned} {
			if counts[state] > 0 {
